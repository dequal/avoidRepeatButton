//
//  UIButton+EQAvoidRepeatButton.m
//  AvoidRepeatButton
//
...
#define DEFAULT_INTERVAL 0.001f  //默认时间间隔
...
@implementation UIButton (EQAvoidRepeatButton)
...
- (void)heSendAction:(SEL)action to:(id)target forEvent:(UIEvent *)event {
    ...
    if ([NSStringFromClass(self.class) isEqualToString:@"UIButton"]) {
        ...
        // 2. 赋值间隔时间
        self.eq_timeInterval = self.eq_timeInterval == 0 ? DEFAULT_INTERVAL : self.eq_timeInterval;
        
        // 3. 判断是否需要忽视重复点击
        if (self.isIgnoreEvent) return;
        // 4. 判断是否间隔时间大于0, 大于0的时候在经过间隔时间之后设置isIgnoreEvent的值为NO(不忽视点击, 就是允许重复点击)
        if (self.eq_timeInterval > 0) {
            [self performSelector:@selector(changeIgnoreState) withObject:self afterDelay:self.eq_timeInterval];
        }
...
#pragma mark -
#pragma mark - eq_timeInterval的setter, getter
- (void)setEq_timeInterval:(NSTimeInterval)eq_timeInterval {
    objc_setAssociatedObject(self, @selector(eq_timeInterval), @(eq_timeInterval), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}


- (NSTimeInterval)eq_timeInterval {
    /*
     _cmd 代表本方法的SEL, 就是 @selector(eq_timeInterval)
